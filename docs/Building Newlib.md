Building the Newlib C library for YOS
==============================================

Introduction
------------

This document contains the instructions necessary to build the [Newlib](http://sourceware.org/newlib/) C library for YOS. The latest version of Newlib as of this writing is 2.4.0

Newlib gives YOS access to the standard set of C library calls like `printf()`, `scanf()`, `memcpy()`, etc.
...
Download the latest YOS source code from GitHub:

	wget https://github.com/fifc/yos/zipball/master
...
Copy the contents of the `newlib/baremetal` directory from the YOS code into the `newlib/libc/sys/baremetal` directory.
